...
def test_standard_true_batching(mock_torch_backbone):
    ...
    with patch.object(VieNeuTTS, '_warmup_model'), \
        ...
        with patch.object(tts, '_decode', return_value=np.zeros(1000)):
            ...
            # Verify backbone.generate was called once for the whole batch
            mock_torch_backbone["model"].generate.assert_called_once()
            # Verify tokenizer.pad was called
            mock_torch_backbone["tokenizer"].pad.assert_called_once()
